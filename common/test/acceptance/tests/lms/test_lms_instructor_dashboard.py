...
@ddt.ddt
class DataDownloadsTestWithMultipleRoles(BaseInstructorDashboardTest):
    """
    Bok Choy tests for the "Data Downloads" tab.
    """
    def setUp(self):
        super(DataDownloadsTestWithMultipleRoles, self).setUp()
        self.course_fixture = CourseFixture(**self.course_info).install()
    ...
    def test_list_student_profile_information(self, role):
        ...
        When I click "List enrolled students' profile information"
            ...
            | staff         |
        """
        self.username, self.id, self.email, self.password = self.log_in_as_instructor(
            global_staff=False,
        ...
        self.data_download_section.generate_enrolled_student_profile_button.click()
        student_profile_info = self.data_download_section.student_profile_information

        self.assertNotIn(student_profile_info, [u'', u'Loading'])
        expected_data = [self.id, self.username, self.email]
    ...
    def test_list_student_profile_information_for_large_course(self, role):
        ...
        self.assertTrue(self.data_download_section.check_enrolled_student_profile_button_presence)
        LogoutPage(self.browser).visit()
        for __ in range(5):
            self.learner_username = "test_student_{uuid}".format(uuid=self.unique_id[0:8])
            self.learner_email = "{user}@example.com".format(user=self.learner_username)
        ...
        self.assertFalse(self.data_download_section.check_enrolled_student_profile_button_presence)
...
length=0""".format(self.course_id)
        ...
        self.data_download_section.generate_grading_configuration_button.click()
        self.assertEqual(self.data_download_section.grading_config_text, expected)
